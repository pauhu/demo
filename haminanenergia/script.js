...
function showExplainerRequest(){
  const q = document.getElementById("explainQ").value || "Why shift load?";
  const req = {
    endpoint: "https://api.anthropic.com/v1/messages",
    model: "claude-3-haiku",
    messages: [
      { role:"system", content:"Explain grid optimization decisions in plain language." },
      { role:"user", content: q }
    ]
  };
  document.getElementById("expReq").textContent = JSON.stringify(req, null, 2);
}
...
// Auto-run ROI calculation at startup
